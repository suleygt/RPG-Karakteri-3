...
export default function App() {
    ...
    Bu video oyunu karakter yaratıcısının bir başlangıç karakterine ihtiyacı var. Göreviniz aşağıdakileri yaparak bir tane oluşturmak: 
      ...
      |               |     - strength: z                  |
			¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯ 	
               ...
               * Kendi karakter adınızı ve saldırı seçeneklerinizi yazabilir veya bu dosyaya zaten içeri aktarılmış olan namesList ve
       attackOptionsList array'lerdekini kullanabilirsiniz.
                  
                ** stats için x, y ve z;  0 ile 100 (dahil) arasında herhangi bir sayı olmalıdır.
                
        3. Kodunuzu çalıştırdığınızda karakterinizin belirttiğiniz özellik değerleriyle ekranda göründüğünü görmelisiniz. Kodunuzu, 
        characterData özellik değerlerinden bazılarını manuel olarak değiştirerek ve doğru sonuçları alıp almadığınızı görerek test etmelisiniz.
         Ayrıca, state ayarlama fonksiyonunu kullanarak değerleri rastgele değiştirecek olan "Değiştir" butonuna da tıklamalısınız
    */
  ...
  /* ️⬇️️ ------------------ Kodunuzu aşağıya yazın -----------------️️ ⬇️️ */
    ...
    const [characterData, setCharacterData] = useState({
      ...
      shield: false,
      weapon: 'sword',
      attackOptions: ['Slash', 'Thrust', 'Block', 'Dodge', 'Parry', 'Counter'],
      stats: {
        hp: 100,
        mp: 50,
        strength: 20
    ...
    const changeCharacterData = () => {
      setCharacterData({
        ...
        hat: getRandomBoolean(),
        shield: getRandomBoolean(),
        weapon: getRandomItem(['sword', 'staff']),
        attackOptions: attackOptionsList.sort(() => Math.random() - 0.5),
        stats: {
          hp: getRandomNumber(),
          mp: getRandomNumber(),
